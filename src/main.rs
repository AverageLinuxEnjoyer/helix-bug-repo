...
fn parser() -> impl Parser<char, Expr, Error = Simple<char>> {
    ...
    let expr = recursive(|expr| {
        let int = text::int(10)
            .map(|int: String| Expr::Num(int.parse().unwrap()))
            .padded();
...
fn eval<'a>(
    ...
    match expr {
        Expr::Num(n) => Ok(*n),
        Expr::Neg(n) => Ok(-eval(n, vars, funcs)?),
        Expr::Add(l, r) => Ok(eval(l, vars, funcs)? + eval(r, vars, funcs)?),
        Expr::Sub(l, r) => Ok(eval(l, vars, funcs)? - eval(r, vars, funcs)?),
        Expr::Mul(l, r) => Ok(eval(l, vars, funcs)? * eval(r, vars, funcs)?),
        Expr::Div(l, r) => Ok(eval(l, vars, funcs)? / eval(r, vars, funcs)?),
        Expr::Var(name) => {
...
fn main() -> anyhow::Result<()> {
    let src = std::fs::read_to_string(
        std::env::args()
            .nth(1)
            .ok_or(anyhow::anyhow!("No argument provided."))?,
    )?;

    match parser().parse(src) {
        Ok(ast) => match eval(&ast, &mut Vec::new(), &mut Vec::new()) {
            Ok(n) => println!("Output: {}", n),
